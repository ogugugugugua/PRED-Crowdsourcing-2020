  ...
  render: ->
    ...
    <div className="subject-set-toolbar">
      ...
      <div className="subject-set-toolbar-links">
        <a className={"toggle-pan-zoom" + if @state.active_pane == 'pan-zoom' then ' active' else '' } onClick={() => @togglePane 'pan-zoom'}><div className="helper">Afficher l'outil de zoom</div></a>
        ...
        <a className={if @props.hideOtherMarks == true then 'fa fa-toggle-on fa-2x' else 'fa fa-toggle-off fa-2x' } onClick={@props.toggleHideOtherMarks}><div className="helper">{if @props.hideOtherMarks == false then "Afficher les marques d'autres contributeurs" else "Afficher seulement vos marques"}</div></a>
      </div>
    </div>
